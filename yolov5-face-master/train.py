...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box',
                    'obj', 'cls', 'landmark', 'total', 'targets', 'img_size'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    wandb.log({"Mosaics": [wandb.Image(
                        str(x), caption=x.name) for x in save_dir.glob('train*.jpg')]})
    ...
    if rank in [-1, 0]:
        ...
        if plots:
            ...
            if wandb:
                files = ['results.png', 'precision_recall_curve.png',
                         'confusion_matrix.png']
                wandb.log({"Results": [wandb.Image(str(save_dir / f), caption=f) for f in files
                                       if (save_dir / f).exists()]})
                if opt.log_artifacts:
                    wandb.log_artifact(artifact_or_path=str(
                        final), type='model', name=save_dir.stem)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='yolov5-face-master/weights/yolov5s.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='yolov5-face-master/models/yolov5s.yaml', 
                        help='model.yaml path')
    parser.add_argument('--data', type=str, default='yolov5-face-master/data/rm.yaml', 
                        help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='yolov5-face-master/data/hyp.scratch.yaml', 
    ...
    parser.add_argument('--multi-scale', action='store_true',
                        default=False, help='vary img-size +/- 50%%')
