...
def detect(model, img0):
    ...
    if pred is not None:
        ...
        for j in range(pred.size()[0]):
            ...
            x2 = int(xywh[0] * w + 0.5 * xywh[2] * w)
            y2 = int(xywh[1] * h + 0.5 * xywh[3] * h)
            boxes.append([x1, y1, x2-x1, y2-y1, conf,int(class_num)])
    return boxes
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='runs/train/exp2/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--save_folder', default='runs/test',
                        type=str, help='Dir to save txt results')
    parser.add_argument('--dataset_folder',
                        default='Dataset/test', type=str, help='dataset path')
    parser.add_argument(
        '--folder_pict', default='Dataset/val.txt', type=str, help='folder_pict')
    ...
    with torch.no_grad():
        ...
        for image_path in tqdm(glob.glob(os.path.join(testset_folder, '*'))):
            ...
            save_name = os.path.join(
                opt.save_folder, pict_folder[image_name], txt_name)
            ...
            with open(save_name, "w") as fd:
                ...
                fd.write(bboxs_num)
                for box in boxes:
                    fd.write('%d %d %d %d %.03f %d' % (
                        box[0], box[1], box[2], box[3], box[4] if box[4] <= 1 else 1, box[5]) + '\n')
        print('done.')
