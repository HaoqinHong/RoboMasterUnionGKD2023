...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            path, imgsz, batch_size, model.stride.max(), opt, pad=0.5, rect=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # Run NMS
            # to pixels
            targets[:, 2:6] *= torch.Tensor([width,
        ...
        for si, pred in enumerate(output):
            # throw landmark in thresh
            pred = torch.cat((pred[:, :5], pred[:, 15:]), 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str,
                        default='yolov5-face-master/data/rm.yaml', help='*.data path')
