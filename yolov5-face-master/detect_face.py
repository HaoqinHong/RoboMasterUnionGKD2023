...
def show_results(img, xyxy, conf, landmarks, class_num):
    ...
    cv2.putText(img, labels[int(class_num)], (x2+2, y2 - 2), 0, tl / 3,
                [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    return img
...
def detect(
    ...
    for path, im, im0s, vid_cap in dataset:
        ...
        imgsz = check_img_size(
            img_size, s=model.stride.max())  # check img_size
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                det[:, 5:15] = scale_coords_landmarks(
                    img.shape[2:], det[:, 5:15], im0.shape).round()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str,
                        default='Dataset2/test', help='source')
    ...
    parser.add_argument('--save-img', default='store_true',
                        help='save results')
    parser.add_argument('--view-img', default='store_true',
                        help='show results')
    opt = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_model(opt.weights, device)
    detect(model, opt.source, device, opt.project, opt.name,
           opt.exist_ok, opt.save_img, opt.view_img)
