...
origin_dataset_path = 'label'
output_dir_path = 'label2'
...
for file in dataset:
    ...
    if len(input.read()) == 0:
        ...
        for line in one_file_data:
            ...
            w = xmax - xmin
            h = ymax - ymin
            output.write(str(names) + " " + str(round(xcenter, 6)) + " " +
                         str(round(ycenter, 6)) + " " + str(round(w, 6)) + " " + str(round(h, 6)))
            for i in range(1, 9):
                output.write(" " + str(line[i]))
            output.write(" " + str(round(xcenter, 6)) + " " + str(round(ycenter, 6)))
